  ...
  return (
    <Modal
      title={intl.formatMessage({
        id: 'pages.chat.sessionMemory',
        defaultMessage: 'Session Memory',
      })}
      open={open}
      onCancel={onClose}
      footer={null}
            ...
            {runs.map((r: any) => (
              <div key={r.id} className={styles.runItem}>
                ...
                <div className={styles.runPreview}>
                  ...
                  {r.userMessage?.substring(0, 100)}
                </div>
                {r.assistantReply && (
                  <div className={styles.runReply}>
                    <span style={{ fontWeight: 500 }}>Assist:</span>{' '}
                    {r.assistantReply.substring(0, 100)}
                  </div>
                )}
              </div>
            ))}
