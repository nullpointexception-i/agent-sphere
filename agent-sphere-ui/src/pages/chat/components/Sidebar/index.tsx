...
export default function Sidebar({
  ...
  const [runDrawerSessionId, setRunDrawerSessionId] = useState<number | null>(
    null,
  );
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      ...
      message.success(
        intl.formatMessage({
          id: 'pages.chat.saveSummary',
          defaultMessage: 'Save Summary',
        }),
      );
    } catch {
      message.error(
              ...
              {selectMode && (
                <span
                  style={{
                    ...
                    fontSize: 16,
                    cursor: 'pointer',
                    color: selectedIds.has(s.id) ? '#1677ff' : undefined,
                  }}
                >
                  {selectedIds.has(s.id) ? '✓' : '○'}
                </span>
              )}
              <span className="conv-label" title={s.title}>
                {s.title}
              </span>
  ...
  return (
    <>
      <div className={styles.sidebarNewChat}>
        ...
        >
          <div
            className={styles.sidebarNewChatItem}
            style={{ display: 'flex', alignItems: 'center', gap: 6 }}
            onClick={onNewChat}
          >
            <PlusOutlined />
            {intl.formatMessage({
              id: 'pages.chat.newChat',
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <span
              className={styles.sidebarNewChatItem}
              style={{ color: selectMode ? '#1677ff' : undefined }}
              onClick={() => !selectMode && setSelectMode(true)}
            >
              <CheckSquareOutlined style={{ marginRight: 4 }} />
              {intl.formatMessage({
                id: 'pages.chat.batchOperation',
            ...
            {selectMode && (
              <div style={{ display: 'flex', gap: 12, marginLeft: 'auto' }}>
                <span
                  className={styles.sidebarNewChatItem}
                  style={{
                    color: selectedIds.size === 0 ? undefined : '#ff4d4f',
                  ...
                  onClick={() => {
                    ...
                    modal.confirm({
                      title: intl.formatMessage(
                        ...
                        { name: `${selectedIds.size} sessions` },
                      ),
                      content: intl.formatMessage(
                        {
                          id: 'pages.deleteConfirm.content',
      ...
      <div className={styles.sidebarScroll}>
        <Conversations
          ...
          onActiveChange={(key) => {
            if (!selectMode) onSelect(Number(key));
          }}
        />
        {hasMoreSessions && (
          ...
          <tbody>
            {[
              ...
              },
              {
                ...
                value: viewSession?.createdBy || '-',
              },
              {
                label: intl.formatMessage({ id: 'pages.table.created' }),
                value: formatTime(viewSession?.createdAt),
              },
              {
                label: intl.formatMessage({
              ...
              },
              {
                ...
                value: formatTime(viewSession?.updatedAt),
              },
            ].map((row) => (
              <tr key={row.label}>
                <td
