...
export default function CliList() {
      ...
      render: (_: any, record: any) => (
        <>
          ...
          <Button
            ...
            onClick={() => {
              modal.confirm({
                title: intl.formatMessage({ id: 'pages.deleteConfirm.title', defaultMessage: 'Delete {name}' }, { name: 'CLI' }),
                content: intl.formatMessage({ id: 'pages.deleteConfirm.content', defaultMessage: 'Are you sure you want to delete this {name}?' }, { name: 'CLI' }),
                okType: 'danger',
                onOk: async () => {
  ...
  const handleBatchDelete = () => {
    ...
    modal.confirm({
      title: intl.formatMessage({ id: 'pages.deleteConfirm.title', defaultMessage: 'Delete {name}' }, { name: `${selectedRowKeys.length} CLI tools` }),
      content: intl.formatMessage({ id: 'pages.deleteConfirm.content', defaultMessage: 'Are you sure you want to delete this {name}?' }, { name: 'CLI tool' }),
      okType: 'danger',
      onOk: async () => {
  ...
  return (
      ...
      />
          <Modal
            title={editing ? intl.formatMessage({ id: 'pages.modal.editCli', defaultMessage: 'Edit CLI' }) : intl.formatMessage({ id: 'pages.modal.newCli', defaultMessage: 'New CLI' })}
            open={modalOpen}
            onOk={handleSubmit}
            onCancel={() => {
              setModalOpen(false);
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="commandTemplate" label={labelWithRule(intl.formatMessage({ id: 'pages.capabilities.commandTemplate', defaultMessage: 'Command Template' }), intl.formatMessage({ id: 'pages.hint.url' }))} rules={[{ required: true }]}>
            <Input.TextArea rows={3} maxLength={500} />
          </Form.Item>
          <Form.Item name="paramSchema" label={labelWithRule(intl.formatMessage({ id: 'pages.capabilities.paramSchema', defaultMessage: 'Parameter Schema' }), intl.formatMessage({ id: 'pages.hint.text' }))}>
            <Input.TextArea rows={6} maxLength={5000} />
          </Form.Item>
          <Form.Item name="workingDir" label={labelWithRule(intl.formatMessage({ id: 'pages.capabilities.workingDir', defaultMessage: 'Working Directory' }), intl.formatMessage({ id: 'pages.hint.url' }))} rules={[{ required: true }]}>
            <Input maxLength={500} />
          </Form.Item>
        </Form>
      </Modal>
