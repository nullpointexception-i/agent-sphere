import { App, Button, Drawer, Form, Input, Modal, Table, Tabs, Tag, Upload } from 'antd';
...
export default function InstanceEditDrawer({ record, open, onClose, onSaved }: Props) {
  ...
  const unmarkRemove = (id: number) => {
    setToRemoveIds((prev) => { const next = new Set(prev); next.delete(id); return next; });
  };
  ...
  const onPick = (type: string, item: any) => {
    setAddCaps((prev) => [...prev, { id: item.id, name: item.name, capabilityType: type }]);
  };

  const onUnpick = (type: string, item: any) => {
    setAddCaps((prev) => prev.filter((c) => !(c.capabilityType === type && c.id === item.id)));
  };

  const removeAdded = (type: string, id: number) => {
    setAddCaps((prev) => prev.filter((c) => !(c.capabilityType === type && c.id === id)));
  };
