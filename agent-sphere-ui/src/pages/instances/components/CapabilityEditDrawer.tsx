...
export default function CapabilityEditDrawer({
  record,
  open,
  onClose,
  onSaved,
      ...
      >
        ...
        >
          {intl.formatMessage({
            ...
            defaultMessage: 'Changes take effect only after clicking Save.',
          })}
        </div>
        <Tabs
          tabPlacement="left"
          items={capTypes.map((type) => {
            ...
            const autoBuiltinIds = new Set(
              allBuiltin.filter((b) => b.needConfig === false).map((b) => b.id),
            );
            const autoRows =
              type === 'builtin'
                ? allBuiltin
            ...
            const userBound =
              type === 'builtin'
                ? bound.filter((b) => !autoBuiltinIds.has(b.capabilityId))
                : bound;
            return {
              key: type,
                    ...
                    columns={[
                      {
                        ...
                        render: (_: any, r: any) => {
                          ...
                          const removed =
                            r.capabilityId != null && toRemoveIds.has(r.id);
                          return (
                            <span
                              style={
                                removed
        ...
        >
          <CapabilityPicker
            ...
            onSelect={(item) =>
              setAddCaps((prev) => [
                ...prev,
                { id: item.id, name: item.name, capabilityType: addingType },
              ])
            }
            onDeselect={(item) =>
              setAddCaps((prev) =>
                prev.filter(
                  (c) => !(c.capabilityType === addingType && c.id === item.id),
                ),
              )
            }
          />
