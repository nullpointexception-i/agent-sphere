export default {
  'pages.layouts.userLayout.title':
    'Ant Design is the most influential web design specification in Xihu district',
  'chat.footer.hint': 'Shift+Enter for new line · Enter to send',
  'pages.login.subtitle': 'Intelligent Agent Management & AI Orchestration',
  'pages.login.accountLogin.tab': 'Account Login',
  'pages.login.accountLogin.errorMessage': 'Incorrect username/password',
  'pages.login.failure': 'Login failed, please try again!',
  'pages.login.success': 'Login successful!',
  'pages.login.username.placeholder': 'Username',
  'pages.login.username.required': 'Please input your username!',
  'pages.login.password.placeholder': 'Password',
  'pages.login.password.required': 'Please input your password!',
  'pages.login.phoneLogin.tab': 'Phone Login',
  'pages.login.phoneLogin.errorMessage': 'Verification Code Error',
  'pages.login.phoneNumber.placeholder': 'Phone Number',
  'pages.login.phoneNumber.required': 'Please input your phone number!',
  'pages.login.phoneNumber.invalid': 'Phone number is invalid!',
  'pages.login.captcha.placeholder': 'Verification Code',
  'pages.login.captcha.required': 'Please input verification code!',
  'pages.login.phoneLogin.getVerificationCode': 'Get Code',
  'chat.empty.title': 'Start a New Conversation',
  'chat.empty.desc':
    'Select an existing session from the sidebar, or click "New Chat" to start a new conversation with an AI agent.',
  'chat.reasoning.show': 'Show thinking',
  'chat.reasoning.hide': 'Hide thinking',
  'chat.reasoning.label': 'Reasoning',
  'chat.showMore': 'Show more',
  'chat.showLess': 'Show less',
  'chat.loadMoreSessions': 'Load more sessions',
  'chat.loadMoreMessages': 'Load more messages',
  'chat.loadMore': 'Load More',
  'chat.copied': 'Copied',
  'chat.intent.retry': 'Retrying with current model...',
  'chat.intent.fallback': 'Switching to fallback model...',
  'pages.deleteConfirm.title': 'Delete {name}',
  'pages.deleteConfirm.content': 'Are you sure you want to delete this {name}?',
  'pages.models.providers': 'Providers',
  'pages.models.apiKeys': 'API Keys',
  'pages.models.modelRoutes': 'Model Routes',
  'pages.table.id': 'ID',
  'pages.table.name': 'Name',
  'pages.table.description': 'Description',
  'pages.table.status': 'Status',
  'pages.table.created': 'Created',
  'pages.table.actions': 'Actions',
  'pages.table.type': 'Type',
  'pages.table.capabilityId': 'Capability ID',
  'pages.table.alias': 'Alias',
  'pages.table.keyValue': 'Key Value',
  'pages.table.expires': 'Expires',
  'pages.table.modelName': 'Model Name',
  'pages.table.weight': 'Weight',
  'pages.table.maxInput': 'Max Input',
  'pages.table.maxOutput': 'Max Output',
  'pages.table.fallbackIds': 'Fallback IDs',
  'pages.table.apiKey': 'API Key',
  'pages.table.createdBy': 'Created By',
  'pages.table.disabled': 'Off',
  'pages.table.enabled': 'On',
  'pages.table.updatedBy': 'Updated By',
  'pages.table.updatedAt': 'Updated At',
  'pages.table.activeKey': 'Active Key',
  'pages.form.name': 'Name',
  'pages.form.description': 'Description',
  'pages.form.maxLength': 'Max {max} characters',
  'pages.form.minLength': 'Min {min} characters',
  'pages.hint.name': '(1-64 chars)',
  'pages.hint.password': '(6-32 chars)',
  'pages.hint.username': '(5-32 letters/digits)',
  'pages.hint.description': '(max 255)',
  'pages.hint.text': '(max 5000)',
  'pages.hint.url': '(max 500)',
  'pages.hint.keyValue': '(max 2000)',
  'pages.hint.englishName': '(1-64 chars, letters/spaces)',
  'pages.instances.systemPrompt': 'System Prompt',
  'pages.instances.businessType': 'Business Type',
  'pages.instances.businessType.extra':
    'Tasks match this instance by businessType; must match the caller',
  'pages.instances.defaultModelRoute': 'Default Model Route',
  'pages.instances.selectRoute': 'Select route',
  'pages.instances.refreshRoutes': 'Refresh routes',
  'pages.instances.routeNoApiKey':
    'The provider for this route has no API key configured',
  'pages.instances.configureApiKey': 'Configure API Key',
  'pages.instances.emptyCapabilities': 'No capabilities bound.',
  'pages.instances.maxLoopCount': 'Max Loop Count',
  'pages.instances.maxLoopCount.extra':
    'Max loops per run; 0=system default, >0=override, required',
  'pages.instances.config': 'Advanced Config (JSON)',
  'pages.instances.config.extra':
    'Sampling params JSON (same shape as Completions config); empty = use global system default',
  'pages.instances.image': 'Image',
  'pages.capabilities.definition': 'Definition',
  'pages.capabilities.skill.tabs.mine': 'My Skills',
  'pages.capabilities.skill.tabs.hub': 'Skill Hub',
  'pages.capabilities.skill.visibility': 'Visibility',
  'pages.capabilities.skill.visibilityPublic': 'Public',
  'pages.capabilities.skill.visibilityPrivate': 'Private',
  'pages.capabilities.skill.publish': 'Publish',
  'pages.capabilities.skill.unpublish': 'Unpublish',
  'pages.capabilities.skill.publishConfirm':
    'Publish "{name}" to the Skill Hub?',
  'pages.capabilities.skill.unpublishConfirm':
    'Unpublish "{name}"? Installed copies are unaffected.',
  'pages.capabilities.skill.install': 'Install',
  'pages.capabilities.skill.installConfirm': 'Install "{name}" to my skills?',
  'pages.capabilities.skill.installConfirmContent':
    'A copy will be added to your skills (private by default) and can be edited independently.',
  'pages.capabilities.skill.installSuccess': 'Installed to my skills',
  'pages.capabilities.skill.author': 'Author',
  'pages.capabilities.skill.installCount': 'Installs',
  'pages.capabilities.serverUrl': 'Server URL',
  'pages.capabilities.toolDefinitions': 'Tool Definitions (JSON)',
  'pages.capabilities.commandTemplate': 'Command Template',
  'pages.capabilities.paramSchema': 'Parameter Schema (JSON)',
  'pages.capabilities.responseSchema': 'Response Schema (JSON)',
  'pages.capabilities.viewSchema': 'View Schema',
  'pages.capabilities.workingDir': 'Working Directory',
  'pages.models.baseUrl': 'Base URL',
  'pages.models.company': 'Company',
  'pages.models.config': 'Config (JSON)',
  'pages.models.fallbackIds': 'Fallback IDs (comma separated)',
  'pages.models.fallbackModels': 'Fallback Models',
  'pages.models.maxInputTokens': 'Max Input Tokens (k)',
  'pages.models.maxOutputTokens': 'Max Output Tokens (k)',
  'pages.models.maxInputPlaceholder': 'e.g. 128',
  'pages.models.maxOutputPlaceholder': 'e.g. 16',
  'pages.models.selectFallback': 'Select fallback routes',
  'pages.models.selectCompany': 'Select company',
  'pages.models.supportsAttachment': 'Attachment',
  'pages.models.supportsAttachmentHint':
    'Allow this route to accept image attachments in chat',
  'pages.models.keyInUse': 'In Use',
  'pages.models.keySetSuccess': 'Set as active key',
  'pages.models.keyUnset': 'Deactivated',
  'pages.models.apiKeyHint':
    'Without a default API Key, routes cannot be used to configure models for instances',
  'pages.models.cancelActive': 'Deactivate',
  'pages.models.setAsActive': 'Set as Active',
  'pages.models.emptyPlaceholder':
    'Select a provider to view API Keys and Routes',
  'pages.dashboard.chat.title': 'Chat',
  'pages.dashboard.chat.desc': 'Start conversation',
  'pages.dashboard.instances.title': 'Instances',
  'pages.dashboard.instances.desc': 'Manage agents',
  'pages.dashboard.models.title': 'Model Providers',
  'pages.dashboard.capabilities.title': 'Capabilities',
  'pages.dashboard.capabilities.desc': 'MCP / Skill / CLI',
  'pages.chat.newChat': 'New Chat',
  'pages.chat.title': 'Title',
  'pages.chat.type': 'Type',
  'pages.chat.sessionDetail': 'Session Detail',
  'pages.chat.selectInstance': 'Select Instance',
  'pages.chat.selectInstancePlaceholder': '-- Select Instance --',
  'pages.chat.sessionMemory': 'Session Memory',
  'pages.chat.memory': 'Session Memory',
  'pages.chat.saveSummary': 'Save Summary',
  'pages.chat.summary': 'Summary:',
  'pages.chat.recentRuns': 'Recent Runs:',
  'pages.chat.noRuns': 'No runs found.',
  'pages.chat.runLabel': 'Run #{id}',
  'pages.chat.pleaseSelectInstance': 'Please select an instance',
  'pages.chat.failedCreateSession': 'Failed to create session',
  'pages.chat.renamed': 'Renamed',
  'pages.chat.modelPlaceholder': 'Model',
  'pages.chat.typeMessage': 'Type a message...',
  'pages.chat.typeMessageHint': 'Type a message... (Shift+Enter for new line)',
  'pages.chat.attachImage': 'Attach image',
  'pages.chat.attachmentTypeHint':
    'Only jpeg/png/webp/gif images are supported',
  'pages.chat.attachmentSizeHint': 'Image must not exceed 5MB',
  'pages.chat.attachmentUploadFailed': 'Attachment upload failed',
  'pages.chat.attachmentBusyHint':
    'There is already a pending attachment. Send or remove it before pasting.',
  'pages.chat.userMessage': 'User Message',
  'pages.chat.runList': 'Runs',
  'pages.chat.interactionType': 'Interaction Type',
  'pages.chat.model': 'Model',
  'pages.chat.durationMs': 'Duration',
  'pages.chat.success': 'Success',
  'pages.chat.collapseDetail': 'Collapse',
  'pages.chat.expandDetail': 'Detail',
  'pages.chat.interactionsForRun': 'Interactions for Run #{runId}',
  'pages.chat.requestBody': 'Request Body',
  'pages.chat.responseBody': 'Response Body',
  'pages.chat.errorMessage': 'Error',
  'pages.chat.detail': 'Detail',
  'pages.modal.newInstance': 'New Instance',
  'pages.modal.editInstance': 'Edit Instance',
  'pages.modal.newCli': 'New CLI Tool',
  'pages.modal.editCli': 'Edit CLI Tool',
  'pages.modal.newSkill': 'New Skill',
  'pages.modal.editSkill': 'Edit Skill',
  'pages.modal.newMcp': 'New MCP',
  'pages.modal.editMcp': 'Edit MCP',
  'pages.modal.newProvider': 'New Provider',
  'pages.modal.editProvider': 'Edit Provider',
  'pages.modal.newApiKey': 'New API Key',
  'pages.modal.editApiKey': 'Edit API Key',
  'pages.modal.newRoute': 'New Route',
  'pages.modal.editRoute': 'Edit Route',
  'pages.instances.tabInfo': 'Info',
  'pages.instances.tabCapabilities': 'Capabilities',
  'pages.instances.detailId': 'ID',
  'pages.instances.detailName': 'Name',
  'pages.instances.detailDescription': 'Description',
  'pages.instances.detailStatus': 'Status',
  'pages.instances.detailCreated': 'Created',
  'pages.instances.detailSystemPrompt': 'System Prompt:',
  'pages.instances.addCapability': '+ Add',
  'pages.instances.promptType': 'Capability type (mcp/skill/cli):',
  'pages.instances.promptId': 'Capability ID:',
  'pages.instances.added': 'Added',
  'pages.instances.removed': 'Removed',
  'pages.instances.removeCapability': 'Remove',
  'pages.instances.undoRemove': 'Undo',
  'pages.instances.batchOperation': 'Batch operations, e.g. batch delete',
  'pages.search.placeholder': 'Search...',
  'pages.dateRange.placeholder': 'Select date range',
  'pages.dateRange.warning': 'Time range cannot exceed 3 months',
  'pages.chat.expandInput': 'Expand Input',
  'pages.chat.selectModel': 'Select Model',
  'pages.chat.deleteSelected': 'Delete',
  'pages.chat.cancel': 'Cancel',
  'pages.chat.batchOperation': 'Batch operations',
  'pages.getCaptchaSecondText': 'sec(s)',
  'pages.login.rememberMe': 'Remember me',
  'pages.login.forgotPassword': 'Forgot Password ?',
  'pages.login.submit': 'Login',
  'pages.login.loginWith': 'Login with :',
  'pages.login.registerAccount': 'Register Account',
  'pages.welcome.link': 'Welcome',
  'pages.welcome.celebrationTitle': 'Welcome to Ant Design Pro {v6}',
  'pages.welcome.alertMessage':
    'Faster and stronger heavy-duty components have been released.',
  'pages.welcome.infoCard.umi.title': 'Learn umi',
  'pages.welcome.infoCard.umi.desc':
    'umi is an extensible enterprise-level frontend framework based on routing, supporting both config-based and convention-based routes.',
  'pages.welcome.infoCard.antd.title': 'Learn Ant Design',
  'pages.welcome.infoCard.antd.desc':
    'antd is a React UI component library based on the Ant Design system, mainly for enterprise-level mid-end products.',
  'pages.welcome.infoCard.procomponents.title': 'Learn Pro Components',
  'pages.welcome.infoCard.procomponents.desc':
    'ProComponents provides higher-abstraction template components on top of Ant Design, with one-component-one-page philosophy.',
  'pages.404.subTitle': 'Sorry, the page you visited does not exist.',
  'pages.404.buttonText': 'Back Home',
  'pages.admin.subPage.title': 'This page can only be viewed by Admin',
  'pages.admin.subPage.alertMessage':
    'Umi ui is now released, welcome to use npm run ui to start the experience.',
  'pages.searchTable.createForm.newRule': 'New Rule',
  'pages.searchTable.updateForm.ruleConfig': 'Rule configuration',
  'pages.searchTable.updateForm.basicConfig': 'Basic Information',
  'pages.searchTable.updateForm.ruleName.nameLabel': 'Rule Name',
  'pages.searchTable.updateForm.ruleName.nameRules':
    'Please enter the rule name!',
  'pages.searchTable.updateForm.ruleDesc.descLabel': 'Rule Description',
  'pages.searchTable.updateForm.ruleDesc.descPlaceholder':
    'Please enter at least five characters',
  'pages.searchTable.updateForm.ruleDesc.descRules':
    'Please enter a rule description of at least five characters!',
  'pages.searchTable.updateForm.ruleProps.title': 'Configure Properties',
  'pages.searchTable.updateForm.object': 'Monitoring Object',
  'pages.searchTable.updateForm.ruleProps.templateLabel': 'Rule Template',
  'pages.searchTable.updateForm.ruleProps.typeLabel': 'Rule Type',
  'pages.searchTable.updateForm.schedulingPeriod.title':
    'Set Scheduling Period',
  'pages.searchTable.updateForm.schedulingPeriod.timeLabel': 'Starting Time',
  'pages.searchTable.updateForm.schedulingPeriod.timeRules':
    'Please choose a start time!',
  'pages.searchTable.titleDesc': 'Description',
  'pages.searchTable.ruleName': 'Rule name is required',
  'pages.searchTable.titleCallNo': 'Number of Service Calls',
  'pages.searchTable.titleStatus': 'Status',
  'pages.searchTable.nameStatus.default': 'default',
  'pages.searchTable.nameStatus.running': 'running',
  'pages.searchTable.nameStatus.online': 'online',
  'pages.searchTable.nameStatus.abnormal': 'abnormal',
  'pages.searchTable.titleUpdatedAt': 'Last Scheduled at',
  'pages.searchTable.exception': 'Please enter the reason for the exception!',
  'pages.searchTable.titleOption': 'Option',
  'pages.searchTable.config': 'Configuration',
  'pages.searchTable.subscribeAlert': 'Subscribe to alerts',
  'pages.searchTable.title': 'Enquiry Form',
  'pages.searchTable.new': 'New',
  'pages.searchTable.chosen': 'chosen',
  'pages.searchTable.item': 'item',
  'pages.searchTable.totalServiceCalls': 'Total Number of Service Calls',
  'pages.searchTable.tenThousand': '0000',
  'pages.searchTable.batchDeletion': 'batch deletion',
  'pages.searchTable.batchApproval': 'batch approval',
  'pages.settings': 'Personal Settings',
  'pages.settings.profile': 'Profile',
  'pages.settings.nickname': 'Nickname',
  'pages.settings.englishName': 'English Name',
  'pages.settings.avatar': 'Avatar',
  'pages.settings.save': 'Save',
  'pages.settings.saved': 'Saved',
  'pages.settings.password': 'Change Password',
  'pages.settings.oldPassword': 'Current Password',
  'pages.settings.newPassword': 'New Password',
  'pages.settings.confirmPassword': 'Confirm Password',
  'pages.settings.passwordMismatch': 'Passwords do not match',
  'pages.settings.passwordChanged': 'Password changed',
  'pages.theme': 'Theme Settings',
  'pages.logout': 'Logout',
  'pages.login.register': 'Register',
  'pages.login.pluginDownload': 'Plugin Download',
  'pages.login.pluginStore': 'From Chrome Web Store',
  'pages.login.pluginSiteDownload': 'Download here',
  'pages.login.sso.title': 'Enterprise SSO Sign-in',
  'pages.login.sso.desc':
    'Sign in with your enterprise identity provider. No registration needed.',
  'pages.login.sso.choose': 'Choose a provider',
  'pages.register.subtitle': 'Create your account',
  'pages.register.success': 'Registration successful!',
  'pages.register.repeatPassword': 'Repeat password',
  'pages.register.repeatPasswordRequired': 'Please repeat password',
  'pages.register.usernameTaken': 'Username already exists',
  'pages.register.backToLogin': 'Already have an account? Login',
  'pages.register.submit': 'Register',
  'pages.register.usernamePattern': 'Letters and digits only',
  'pages.register.englishNamePattern':
    'English name must be letters or spaces only',
  'pages.save': 'Save',
  'pages.save.success': 'Saved',
  'pages.cancel': 'Cancel',
  'pages.form.new': 'New',
  'pages.table.edit': 'Edit',
  'pages.table.view': 'View',
  'pages.table.delete': 'Delete',
  'pages.table.empty': 'No data',
  'pages.instances.capabilities': 'Capabilities',
  'pages.instances.modelRoute': 'Model',
  'pages.instances.capabilitiesSaveHint':
    'Changes take effect only after clicking Save.',
  'pages.instances.currentCapabilities': 'Current',
  'pages.instances.autoInclude': 'Built-in',
  'pages.instances.selected': 'Selected',
  'pages.instances.select': 'Select',
  'pages.batchDelete': 'Delete ({count})',
  'pages.batchDelete.success': 'Deleted {count} items',
  'pages.landing.title': "What's new idea today?",
  'pages.landing.more': 'More',
  'pages.landing.selectInstance': 'Select Instance',
  'pages.landing.noInstance': 'No AI instances available',
  'pages.landing.noInstanceDesc': 'Create an instance to start a conversation',
  'pages.landing.createInstance': 'Create Instance',
  'pages.landing.noModelRoute':
    'Please configure a default model route for this instance first',
  'pages.landing.noModelRouteTitle': 'No default model configured',
  'pages.landing.noModelRouteConfirm': 'Configure a model for this instance?',
  'pages.landing.goConfigure': 'Configure',
  'pages.landing.needModelRoute': 'Default model required',
  'pages.chat.tools': 'Tools',
  'pages.chat.sessionPanel': 'Session Panel',
  'pages.chat.todoList': 'Todo List',
  'pages.chat.todoList.empty': 'No todo items yet',
  'pages.chat.toolCalls': 'Tool Calls',
  'pages.chat.toolCalls.empty': 'No tool calls yet',
  'pages.chat.sessionSummary': 'Session Summary',
  'pages.chat.noSummary': 'No summary available',
  'pages.chat.saveFailed': 'Save failed',
  'pages.chat.voiceInput': 'Voice Input',
  'pages.chat.voiceInputStop': 'Click to stop',
  'pages.chat.voiceInputOffline': 'Currently offline, voice input unavailable',
  'pages.chat.voiceInputNotSupported': 'Browser does not support voice input',
  'pages.chat.voiceInputNotAllowed': 'Microphone permission denied',
  'pages.chat.voiceInputNetworkError':
    'Network error, voice recognition failed',
  'pages.chat.voiceInputError': 'Speech recognition failed',
  'pages.chat.voiceInputStartFailed': 'Failed to start voice input',
  'pages.document.title': 'Title',
  'pages.document.preview': 'Preview',
  'pages.document.session': 'Session',
  'pages.document.createdAt': 'Created',
  'pages.document.notFound': 'Document not found',
  'pages.document.back': 'Back',
  'pages.chat.viewDocument': 'View Document',
  'pages.document.edit': 'Edit',
  'pages.document.save': 'Save',
  'pages.document.saveSuccess': 'Saved',
  'pages.document.saveFailed': 'Save failed',
  'pages.document.markdown': 'Markdown',
  'pages.document.summary': 'Summary',
  'pages.document.contentType': 'Content Type',
  'pages.document.updatedAt': 'Updated At',
  'pages.document.viewDetail': 'View',
  'pages.document.previewContent': 'Preview',
  'pages.document.loadFailed': 'Failed to load document',
  'pages.document.deleted': 'Deleted',
  'pages.document.outline': 'Outline',
  'pages.document.noHeadings': 'No headings',
  'pages.document.charCount': 'Chars',
  'pages.document.exported': 'Exported',
  'pages.document.exportFailed': 'Export failed',
  'pages.document.share': 'Share',
  'pages.document.shareTitle': 'Share Document',
  'pages.document.shareFailed': 'Share failed',
  'pages.document.shareHint': 'Scan QR code or copy link to share',
  'pages.document.sharedDocNotFound':
    'Document not found or sharing has been cancelled',
  'pages.document.sharedDocCreated': 'Created',
  'pages.demo.banner':
    'Demo account — all models are free. For complex tasks, use DeepSeek V4 Flash+ or OpenAI 5.1+ for better results.',
  'pages.demo.bannerAction': 'Configure',
  'pages.demo.bannerRegister': 'Register & Configure',
  'pages.admin.settings.title': 'System Settings',
  'pages.admin.backToMain': 'Back to Main',
  'pages.admin.users.title': 'Users',
  'pages.admin.roles.title': 'Roles',
  'pages.admin.permissions.title': 'Permissions',
  'pages.admin.users.assignRole': 'Assign Roles',
  'pages.admin.roles.create': 'New Role',
  'pages.admin.roles.assignPermission': 'Assign Permissions',
  'pages.admin.permissions.create': 'New Permission',
  'pages.admin.settings.group.security': 'Security',
  'pages.admin.settings.group.chrome': 'Chrome Extension',
  'pages.admin.settings.group.web-read': 'Web Read',
  'pages.admin.settings.group.rate-limit': 'Rate Limit',
  'pages.admin.settings.group.plugin': 'Plugin Package',
  'pages.admin.settings.group.user': 'User Init',
  'pages.admin.settings.group.llm': 'LLM Defaults',
  'pages.admin.settings.template.title': 'Resource Template',
  'pages.admin.settings.template.tab.form': 'Form',
  'pages.admin.settings.template.tab.formDesc':
    'Visually edit resource template items by type; providers / routes refer to each other by template names.',
  'pages.admin.settings.template.tab.json': 'JSON',
  'pages.admin.settings.template.tab.jsonDesc':
    'JSON array format, matching backend config storage; must be valid JSON on save.',
  'pages.admin.settings.template.jsonInvalidOnOpen':
    'Current config is not valid JSON. Fix it in JSON mode first.',
  'pages.admin.settings.template.jsonRequired':
    'Must be a valid JSON string; may be left empty on save',
  'pages.admin.settings.template.selectProvider':
    'Select a provider from the template',
  'pages.admin.settings.template.selectRoute':
    'Select a model route from the template',
  'pages.admin.settings.template.selectType': 'Select item type',
  'pages.admin.settings.template.add': 'Add',
  'pages.admin.settings.template.count': 'Total',
  'pages.admin.settings.template.items': 'items',
  'pages.admin.settings.template.delete.confirm': 'Delete this item?',
  'pages.admin.settings.template.col.type': 'Type',
  'pages.admin.settings.template.col.name': 'Name',
  'pages.admin.settings.template.col.config': 'Config',
  'pages.admin.settings.template.col.status': 'Status',
  'pages.admin.settings.template.col.action': 'Actions',
  'pages.admin.settings.template.status.ok': 'OK',
  'pages.admin.settings.template.label.provider': 'Provider',
  'pages.admin.settings.template.label.route': 'Model route',
  'pages.admin.settings.template.label.promptSystem': 'System Prompt',
  'pages.admin.settings.template.label.promptUser': 'User Prompt',
  'pages.admin.settings.template.label.serverType': 'Server Type',
  'pages.admin.settings.template.label.definition': 'Definition',
  'pages.admin.settings.template.label.title': 'Title',
  'pages.admin.settings.template.label.contentType': 'Content Type',
  'pages.admin.settings.template.label.content': 'Content',
  'pages.admin.settings.template.type.modelProvider': 'Model Provider',
  'pages.admin.settings.template.type.apiKey': 'API Key',
  'pages.admin.settings.template.type.modelRoute': 'Model Route',
  'pages.admin.settings.template.type.completions': 'Completions Capability',
  'pages.admin.settings.template.type.instance': 'Agent Instance',
  'pages.admin.settings.template.type.mcp': 'MCP Server',
  'pages.admin.settings.template.type.skill': 'Skill',
  'pages.admin.settings.template.type.document': 'Document',
  'pages.admin.settings.template.skill.promptTemplate':
    'Task instruction (promptTemplate)',
  'pages.admin.settings.template.skill.promptTemplateHint':
    'Supports Markdown and {{field}} placeholders',
  'pages.admin.settings.template.skill.parameters':
    'Input JSON Schema (parameters)',
  'pages.admin.settings.template.skill.parametersHint':
    'May be empty (empty object)',
  'pages.admin.settings.group.sso': 'SSO',
  'pages.admin.settings.plugin.upload.btn': 'Upload Package',
  'pages.admin.settings.plugin.upload.success': 'Uploaded',
  'pages.admin.settings.plugin.delete.btn': 'Delete',
  'pages.admin.settings.plugin.delete.confirm':
    'Delete the managed plugin package? The download entry will be hidden.',
  'pages.admin.settings.plugin.delete.success': 'Deleted',
  'pages.admin.settings.regenerate.btn': 'Regenerate',
  'pages.admin.settings.regenerate.confirm':
    'Regenerating the AES key will re-encrypt all API keys. This action cannot be undone. Continue?',
  'pages.admin.settings.regenerate.success':
    'AES key regenerated, all API keys re-encrypted',
  'pages.admin.settings.edit.label': 'Configuration Value',
  'pages.admin.settings.edit.secret.placeholder':
    'Leave empty to keep current value',
  'pages.admin.users.roles': 'Roles',
  'pages.table.code': 'Code',
  'pages.table.create': 'New',
  'pages.table.sort': 'Sort',
  'pages.form.code': 'Code',
  'pages.form.type': 'Type',
  'pages.form.sort': 'Sort',
  'pages.admin.permissions.parent': 'Parent',
  'pages.admin.permissions.parentPlaceholder': 'None (top-level)',
  'pages.admin.permissions.codePattern': 'Format: module:action',
  'pages.admin.roles.codePattern': 'Uppercase letters and underscores only',
  'pages.admin.settings.secret': 'Secret',
  'pages.admin.settings.notSet': '(not set)',
  'pages.admin.settings.empty': '(empty)',
  'pages.admin.auditLogs.title': 'Audit Logs',
  'pages.admin.auditLogs.time': 'Time',
  'pages.admin.auditLogs.user': 'User',
  'pages.admin.auditLogs.action': 'Action',
  'pages.admin.auditLogs.resourceType': 'Resource Type',
  'pages.admin.auditLogs.resourceId': 'Resource ID',
  'pages.admin.auditLogs.detail': 'Detail',
  'pages.admin.auditLogs.ipAddress': 'IP Address',
  'pages.admin.auditLogs.status': 'Status',
  'pages.admin.auditLogs.success': 'Success',
  'pages.admin.auditLogs.failure': 'Failure',
  'pages.admin.auditLogs.actions.LOGIN': 'Login',
  'pages.admin.auditLogs.actions.LOGOUT': 'Logout',
  'pages.admin.auditLogs.actions.REGISTER': 'Register',
  'pages.admin.auditLogs.actions.CREATE': 'Create',
  'pages.admin.auditLogs.actions.UPDATE': 'Update',
  'pages.admin.auditLogs.actions.DELETE': 'Delete',
  'pages.admin.auditLogs.actions.BATCH_DELETE': 'Batch Delete',
  'pages.admin.auditLogs.actions.ASSIGN_ROLE': 'Assign Role',
  'pages.admin.auditLogs.actions.ASSIGN_PERMISSION': 'Assign Permission',
  'pages.admin.auditLogs.actions.SET_MODEL_ROUTE': 'Set Model Route',
  'pages.admin.auditLogs.actions.SET_ACTIVE_KEY': 'Set Active Key',
  'pages.admin.auditLogs.actions.UPDATE_PROFILE': 'Update Profile',
  'pages.admin.auditLogs.actions.UPDATE_PASSWORD': 'Change Password',
  'pages.admin.auditLogs.actions.REGENERATE': 'Regenerate',
  'pages.admin.auditLogs.actions.PAGE_VIEW': 'Page View',
  'pages.admin.auditLogs.actions.PAGE_EXIT': 'Page Exit',
  'pages.admin.auditLogs.actions.CLICK': 'Click',
  'pages.admin.auditLogs.actions.SELECT': 'Select',
  'pages.admin.auditLogs.actions.DWELL': 'Dwell',
  'pages.admin.auditLogs.search.placeholder': 'Search username',
  'pages.admin.auditLogs.filter.all': 'All Actions',
  'pages.admin.identityProviders.title': 'Identity Providers',
  'pages.admin.identityProviders.create': 'New Identity Provider',
  'pages.admin.identityProviders.edit': 'Edit Identity Provider',
  'pages.admin.identityProviders.code': 'Code',
  'pages.admin.identityProviders.name': 'Name',
  'pages.admin.identityProviders.type': 'Type',
  'pages.admin.identityProviders.issuer': 'Issuer',
  'pages.admin.identityProviders.clientId': 'Client ID',
  'pages.admin.identityProviders.clientSecret': 'Client Secret',
  'pages.admin.identityProviders.clientSecret.keep':
    'Leave empty to keep current value',
  'pages.admin.identityProviders.secretConfigured': 'Configured',
  'pages.admin.identityProviders.authorizationEndpoint':
    'Authorization Endpoint',
  'pages.admin.identityProviders.tokenEndpoint': 'Token Endpoint',
  'pages.admin.identityProviders.jwksUrl': 'JWKS URL',
  'pages.admin.identityProviders.scopes': 'Scopes',
  'pages.admin.identityProviders.enabled': 'Enabled',
  'pages.admin.identityProviders.remark': 'Remark',
  'pages.admin.identityProviders.testConnection': 'Test Connection',
  'pages.admin.identityProviders.testConnection.success':
    'Connection test passed',
  'pages.admin.identityProviders.testConnection.failed':
    'Connection test failed',
  'pages.admin.identityProviders.search.placeholder': 'Search name or code',
  'pages.admin.identityProviders.defaultRole': 'Default Role',
  'pages.admin.identityProviders.defaultRole.extra':
    'Granted to new SSO users (replaces default USER role)',
  'pages.admin.identityProviders.defaultRole.placeholder':
    'Select default role',
  'pages.admin.identityProviders.resourceTemplate': 'Resource Template (JSON)',
  'pages.admin.identityProviders.resourceTemplate.extra':
    'Empty uses the default template; each user gets their own resource copy on first login',
  'pages.admin.identityProviders.viewSample': 'View sample',
  'pages.admin.identityProviders.sample.title':
    'Default Resource Template Sample',
  'pages.admin.identityProviders.copy': 'Copy JSON',
  'pages.admin.copy.success': 'Copied',
  'pages.table.close': 'Close',
  'pages.admin.loading': 'Loading…',
  'pages.table.total': 'Total {n} items',
  'pages.table.detail': 'Detail',
  'pages.table.updated': 'Updated At',
  'pages.periodicTasks.comingSoon': 'Coming soon',
  'pages.artifacts.taskArtifacts.task': 'Task',
  'pages.artifacts.taskArtifacts.type': 'Type',
  'pages.artifacts.taskArtifacts.schemaRef': 'Schema Ref',
  'pages.artifacts.taskArtifacts.runId': 'Run ID',
  'pages.artifacts.taskArtifacts.taskId': 'Task ID',
  'pages.artifacts.taskArtifacts.createdBy': 'Created By',
  'pages.artifacts.taskArtifacts.remark': 'Remark',
  'pages.artifacts.taskArtifacts.content': 'Content',
  'pages.artifacts.taskArtifacts.copy': 'Copy',
  'pages.artifacts.taskArtifacts.copied': 'Copied',
  'pages.artifacts.taskArtifacts.detailTitle': 'Task Artifact Detail',
  'pages.chat.loadFailed': 'Load failed',
  'pages.delete.success': 'Deleted',
  'pages.update.success': 'Updated',
  'pages.create.success': 'Created',
  'pages.admin.completions.title': 'Completions',
  'pages.admin.completions.create': 'New Completions',
  'pages.admin.completions.edit': 'Edit Completions',
  'pages.admin.completions.detail': 'Detail',
  'pages.admin.completions.name': 'Name',
  'pages.admin.completions.description': 'Description',
  'pages.admin.completions.modelRoute': 'Model Route',
  'pages.admin.completions.modelRoute.extra':
    'Leave empty to make this capability non-executable',
  'pages.admin.completions.modelRoute.placeholder': 'Select a model route',
  'pages.admin.completions.noRoute': 'Not bound',
  'pages.admin.completions.activePrompt': 'Active Prompt',
  'pages.admin.completions.status': 'Status',
  'pages.admin.completions.statusActive': 'Active',
  'pages.admin.completions.remark': 'Remark',
  'pages.admin.completions.businessType': 'Business Type',
  'pages.admin.completions.businessType.extra':
    'External API matches this capability by businessType; must match the caller',
  'pages.admin.completions.inputSchema': 'Input Schema (JSON)',
  'pages.admin.completions.outputSchema': 'Output Schema (JSON)',
  'pages.admin.completions.config': 'Config (JSON)',
  'pages.admin.completions.config.help':
    'Supported (optional):\ntemperature: number, e.g. 0.1\nmax_tokens: number\ntop_p: number\npresence_penalty / frequency_penalty: number\nstop: string array, e.g. ["END"]\nthinking / reasoning: false=disable thinking, true=enable, or "disabled"/"enabled"\nExample: temperature=0.1, thinking=false, max_tokens=1024',
  'pages.admin.completions.invalidJson': 'Invalid JSON',
  'pages.admin.completions.search.placeholder': 'Search by name',
  'pages.admin.completions.prompts': 'Prompt Versions',
  'pages.admin.completions.promptDetail': 'Prompt Version Detail',
  'pages.admin.completions.callDetail': 'Call Record Detail',
  'pages.admin.completions.calls.input': 'Input',
  'pages.admin.completions.addPrompt': 'Add Version',
  'pages.admin.completions.promptVersion': 'Version',
  'pages.admin.completions.promptSystem': 'System Prompt',
  'pages.admin.completions.promptUser': 'User Prompt',
  'pages.admin.completions.promptPlaceholder':
    'Supports input and field placeholders',
  'pages.admin.completions.active': 'Active',
  'pages.admin.completions.activate': 'Activate',
  'pages.admin.completions.activatePromptConfirm':
    'Confirm activating this version?',
  'pages.admin.completions.calls': 'Call Records',
  'pages.admin.completions.calls.model': 'Model',
  'pages.admin.completions.calls.caller': 'Caller',
  'pages.admin.completions.calls.usage': 'Usage',
  'pages.admin.completions.calls.output': 'Output',
  'pages.admin.tasks.title': 'One-off Tasks',
  'pages.admin.tasks.create': 'New Task',
  'pages.admin.tasks.detail': 'Task Detail',
  'pages.admin.tasks.goal': 'Goal',
  'pages.admin.tasks.status': 'Status',
  'pages.admin.tasks.statusQueued': 'Queued',
  'pages.admin.tasks.statusRunning': 'Running',
  'pages.admin.tasks.statusCompleted': 'Completed',
  'pages.admin.tasks.statusFailed': 'Failed',
  'pages.admin.tasks.statusCancelled': 'Cancelled',
  'pages.admin.tasks.instanceId': 'Instance',
  'pages.admin.tasks.businessType': 'Business Type',
  'pages.admin.tasks.businessType.extra':
    'Matching available instance by businessType; must match the instance businessType',
  'pages.admin.tasks.instanceId.extra':
    'Leave empty to use first available instance',
  'pages.admin.tasks.instanceId.placeholder': 'Select an instance',
  'pages.admin.tasks.sessionId': 'Session',
  'pages.admin.tasks.runId': 'Run',
  'pages.admin.tasks.callbackUrl': 'Callback URL',
  'pages.admin.tasks.context': 'Context',
  'pages.admin.tasks.expectedOutput': 'Expected Output',
  'pages.admin.tasks.result': 'Result',
  'pages.admin.tasks.remark': 'Remark',
  'pages.admin.tasks.stop': 'Stop',
  'pages.admin.tasks.stopConfirm': 'Confirm stopping this task?',
  'pages.admin.tasks.search.placeholder': 'Search by goal',
  'pages.admin.tasks.filterStatus': 'Filter by status',
};
