...
export default function SetModelRouteModal({
  open,
  instance,
  onClose,
  onSuccess,
}: SetModelRouteModalProps) {
  const intl = useIntl();
  ...
  return (
    <Modal
      title={intl.formatMessage({
        id: 'pages.instances.defaultModelRoute',
        defaultMessage: 'Default Model Route',
      })}
      open={open}
      onCancel={() => {
        onClose();
      ...
      onOk={async () => {
        ...
        setSubmitting(true);
        try {
          await agentApi.instances.setModelRoute(
            instance.id,
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <Select
          ...
          placeholder={intl.formatMessage({
            id: 'pages.instances.selectRoute',
                ...
                {r.apiKeyConfigured === false && (
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      window.open(
                        `/models?openApiKeys=${r.providerId}`,
                        '_blank',
                      );
                    }}
                  >
                    {intl.formatMessage({
                      id: 'pages.instances.configureApiKey',
                      defaultMessage: '配置API Key',
                    })}
                  </Button>
                )}
              </div>
        ...
        <Tooltip
          title={intl.formatMessage({
            id: 'pages.instances.refreshRoutes',
            defaultMessage: '刷新路由',
          })}
        >
